...
const sections:Array<[string,string,string[]?]> = [
 ...
 ["Contributions & Expenses","A Contribution groups expense items. Each item has its own amount, payer, and participants; its amount is divided equally among selected participants.",["Ambagan shows each person's share of a Contribution, calculated from the participants of every individual Expense Item—not merely the Contribution's overall people list.","When every computed share is identical, AmbagGabay shows All (N people) — ₱X each. Friends with matching shares may otherwise be grouped in one row."]],
...
export default function GuidePage(){return <><Link href="/settings" className="back-link"><ArrowLeft size={18}/> Settings</Link><PageHeader title="How to Use AmbagGabay" description="A practical guide to sharing expenses, access, and settlements."/><main className="guide"><QuickStartGuide/><div className="guide-accordions">{sections.map(([title,copy,bullets],index)=><details className="panel" key={title} open={index===0}><summary><span>{title}</span></summary><div className="guide-content"><p>{copy}</p>{bullets&&<ul>{bullets.map(item=><li key={item}>{item}</li>)}</ul>}</div></details>)}</div></main></>}
