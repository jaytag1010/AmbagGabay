...
import { resolveOwnerLinkedIdentity } from "@/utils/sharedIdentity";
export default function ManageSharingPage() {
  ...
  const memberIdentity=(member:import("@/types").FolderMembership)=>resolveOwnerLinkedIdentity(friends.items,member.userId,member.displayNameSnapshot),invitationIdentity=(invite:import("@/types").FolderInvitation)=>resolveOwnerLinkedIdentity(friends.items,invite.recipientUid,invite.recipientNameSnapshot,invite.recipientEmail);
  useEffect(() => {
    ...
    <>
                ...
                <>
                  <select
                    ...
                    onChange={(event) =>
                      changeMemberRole(
                        ...
                        folder,
                        member,
                        event.target.value as "editor" | "viewer",identity.name,
                      )
                    }
                  >
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      if (confirm(active?`Remove ${identity.name} from ${folder.name}?\n\n${identity.name} still has active shared expenses in this Folder.\n\nCurrent balance: ${summary}\n\nRemoving access will not remove or settle these obligations. Historical records and the Friend link will remain intact.`:`Remove ${identity.name} from ${folder.name}?\n\n${identity.name.split(" ")[0]} will lose access to this Folder. Historical Contributions, Expenses, settlements, involvement, balances, and the Friend link will remain intact.`))
                        await removeMember(uid, folder, member,identity.name);
                    }}
                  >
                    Remove
                  </Button>
      ...
      <section className="panel">
        ...
        {!invitations.items.length ? (
          ...
          <div className="share-list">
            {invitations.items.map((invite) => {const identity=invitationIdentity(invite);return (
              <div key={invite.id}>
                <span>
                  <strong>{identity.name}</strong>
                  <small>
                    {identity.email}{identity.email?" · ":""}{invite.role[0].toUpperCase()+invite.role.slice(1)} · Pending
                  </small>
                </span>
                <Button
                  variant="secondary"
                  onClick={() => confirm(`Cancel the invitation to ${identity.name}?`)&&cancelInvitation(uid, invite)}
                >
                  Cancel Invite
                </Button>
              </div>
